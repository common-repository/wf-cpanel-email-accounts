...
//			error_log( [ 'Response: ', $response ] );
	...
	public    static function has_features( array $features = [ 'popaccts', 'forwarders' ] ): bool {
		...
		foreach ( $features as $feature ) {
			$has = $has && ( self::result( 'Features', 'has_feature', [ 'name' => $feature ] ) ?? false );
		}
		return $has;
	}
	...
	public    static function set_locale( string $locale = '' ): bool {
		...
		if ( $change ) {
			self::result( 'Locale', 'set_locale', [ 'locale' => $set_locale ] ) ?? false;
		}
		return $change;
	}

	public    static function server_information( string $name = '' ): array|string|int {

		if ( empty( $name ) ) {
			return self::result( 'ServerInformation', 'get_information' ) ?? [];
		} else {
			return \wp_list_pluck( self::result( 'ServerInformation', 'get_information' ) ?? [], 'value', 'name' )[ $name ] ?? '';
		}
	}

	public    static function server_info( string $name = '' ): array|string|int|null {

		if ( empty( $name ) ) {
			return self::result( 'Variables', 'get_server_information' ) + self::result( 'ServerInformation', 'get_information' );
		} else {
			return self::result( 'Variables', 'get_server_information', [ 'name' => $name ] )->$name;
		}
	}

	public    static function session_info( string $name = '' ): array|string|int|null {

		if ( empty( $name ) ) {
			return self::result( 'Variables', 'get_session_information' );
		} else {
			return self::result( 'Variables', 'get_session_information', [ 'name' => $name ] )->$name ?? null;
		}
	}
	public    static function quotas(): \stdClass {
		return self::result( 'Quota', 'get_local_quota_info' ) ?? ( new \stdClass );
	}
	...
	public    static function user_uid(): int {
		return \intval( self::result( 'Variables', 'get_user_information' )->uid ?? null );
	}

	public    static function user_gid(): int {
		return \intval( self::result( 'Variables', 'get_user_information' )->gid ?? null );
	}
	...
	public    static function two_factor(): bool {
		return \boolval( self::result( 'TwoFactorAuth', 'get_user_configuration' )->is_enabled ?? null );
	}
	...
	public    static function mail_exchangers( string $domain = null ): array|string {
//		return \wp_list_pluck( self::result( 'Email', 'list_mxs', [ 'domain' => $domain ] ), 'entries' )[0];
		return self::result( 'Email', 'list_mxs', [ 'domain' => $domain ] )[0]->mx;
	}

	public    static function validate_spfs( string $domain ) : array {
		return self::result( 'EmailAuth', 'validate_current_spfs', [ 'domain' => $domain ] );
	}

	public    static function validate_ptrs( string $domain ) : array {
		return self::result( 'EmailAuth', 'validate_current_ptrs', [ 'domain' => $domain ] );
	}
	...
	public    static function all_queued_emails(): int {
		...
		foreach ( (array) self::result( 'Email', 'list_pops' ) as $account ) {
			$held += (int) self::queued_emails( $account->email );
		}
		foreach ( (array) self::result( 'Email', 'list_forwarders' ) as $account ) {
			$held += (int) self::queued_emails( $account->dest );
		}
		return $held;
	}
...
//		echo '<pre>'; var_dump( self::result( 'UserManager', 'list_users' ) );echo '</pre>';
		...
		foreach ( (array) self::result( 'UserManager', 'list_users' ) as $account ) {
			...
			if ( ! \str_starts_with( $account->username, self::$cpanel_user ) ) {
				...
				foreach ( $account->services ?? [] as $sname => $service ) {
...
//		return self::result( 'Backup', 'list_backups', [], true );
	...
	private   static function trace_destinations( array $dests, string $address = 'then', bool $last_only = false ): array {
		$key = \array_key_last( $dests );
		$address = $dests[ $key ]->address ?? $address;
		$result = \is_null( $key)  ? [] : [ [ 'address' => Main::email_to_utf8( $address ), 'type' => $dests[ $key ]->type, 'message' => $dests[ $key ]->message ?? '', 'mx' => [ (object) [ 'hostname' => $dests[ $key ]->mx[0]->hostname ?? null ] ], 'mailbox' => $dests[ $key ]->mailbox ?? false ] ];

		if ( \is_object( $dests[ $key ] ) && \property_exists( $dests[ $key ], 'destinations' ) ) {

			if ( $last_only ) {
				$result = self::trace_destinations( $dest_dirs[ $key ]->destinations, $address, $last_only );
			} else {
				$result = \array_merge( $result, self::trace_destinations( $dests[ $key ]->destinations, $address ) );
			}
		}
		return $result;
	}
	...
	public    static function trace( string $email, bool $last_only = false ): string {
		...
		if ( $result->status ) {
			...
			return \implode( ' &rarr; ', \array_map( static function( array $dests ) use( $data, $last_only ): string {
				$types = [
				...
				return ( $last_only ? '' : $dests['address'] . ' ' ) . \WebFacing\cPanel\Email\_x( 'will', 'Trace' ) . ' <mark>' . ( $types[ $data->type ] ?? $data->type ) . '</mark>' . ( ( $dests['message'] ?? '' ) ? ' "' . $dests['message'] . '"' : '' ) . ( ( $dests['mailbox'] ?? '' ) || ( $dests['mx'][0]->hostname ?? '' ) ? ' ' . \WebFacing\cPanel\Email\_x( 'to', 'Trace' ) . ' ' . ( $dests['mailbox'] ?? '' ) . ( $dests['mx'][0]->hostname ?? '' ) : '' );
			}, $dests ? self::trace_destinations( $dests, $data->address, $last_only ) : [] ) );
		} else {
