...
abstract class SiteHealth extends Main {
	...
	public    static function admin(): void {
		...
		\add_filter( 'debug_information', static function( array $debug_info ): array {
			...
			$x = _x( 'Email Delivery Route (deprecated)',     'Site Health cPanel® Feature Name' );
			/* translators: Site Health Info SPF State, Site Health Info PTR State */
			$s = _x( 'VALID',         'Site Health Info' );
			/* translators: Site Health Info SPF State */
			$s = _x( 'MISMATCHED',    'Site Health Info' );
			/* translators: Site Health Info SPF State */
			$s = _x( 'MULTIPLE',      'Site Health Info' );
			/* translators: Site Health Info SPF State */
			$s = _x( 'MISSING',       'Site Health Info' );
			/* translators: Site Health Info PTR State */
			$s = _x( 'IP_IS_PRIVATE', 'Site Health Info' );
			/* translators: Site Health Info PTR State */
			$s = _x( 'MISSING_PTR',   'Site Health Info' );
			/* translators: Site Health Info PTR State */
			$s = _x( 'PTR_MISMATCH',  'Site Health Info' );
			/* translators: Site Health Info SPF State, Site Health Info PTR State */
			$s = _x( 'ERROR',         'Site Health Info' );
			...
			$used = (string) UAPI::server_information( 'Memory Used' );

			if ( $used ) {
				$debug_info['wp-server']['fields']['cp-memory'] = [
					'label'   => _x( 'cPanel® Memory Used', 'Site Health Info' ),
					'value'   => \substr( $used, 0, -2 ) . '%',
					'debug'   => $used,
					'private' => false,
				];
			}
			$load = \floatval( UAPI::server_information( 'Server Load' ) );

			if ( $load ) {
				$debug_info['wp-server']['fields']['cp-load'] = [
					'label'   => _x( 'cPanel® Server Load', 'Site Health Info' ),
					'value'   => $load . ' (' . \number_format_i18n( 100. * $load / \intval( UAPI::server_information( 'CPU Count' ) ?: 1 ), 0 ) . '%)',
					'debug'   => $load,
					'private' => false,
				];
			}
			...
			$debug_info['wp-database']['fields']['threads_connected'] = [
				/* translators: Database Threads connected */
				'label'   => _x( 'Threads connected', 'Site Health Info' ),
				'value'   => \intval( $result['Value'] ?? 0 ),
				'debug'   => $result['Value'] ?? '',
				'private' => false,
			];
...
//			if ( ! ( \defined( 'DISALLOW_FILE_MODS' ) && \DISALLOW_FILE_MODS ) ) {
			if ( \wp_is_file_mod_allowed( self::$screen->id ) ) {
...
//				];
			...
			if ( self::$is_debug || ( \defined( 'WF_CPANEL_HOST' ) && \defined( 'WF_CPANEL_USER' ) ) ) {
			...
			$mail_spf       = UAPI::validate_spfs( self::$site_domain );
			$mail_ptr       = UAPI::validate_ptrs( self::$site_domain );
			$max_emails     = UAPI::maximum_emails();
			$name_servers   = \array_column( \dns_get_record(self::$site_domain, \DNS_NS ), 'target' );
			$held_mails     = UAPI::all_queued_emails();
			$contacts       = UAPI::get_user_info();
			$show_main      = ! self::$domain_only || self::$site_domain === self::$main_domain;
			$subaccounts    = [];
			$addon_domains  = UAPI::list_domains( 'addon_domains' );
				...
				'fields' => [
					...
					'plugin_usage'        => [
						...
						'value'    => ( static function() {
							...
							return \count( $vals ) ? \implode( ', ', $vals ) : __( 'none' );
						} )(),
						'debug'    => \print_r( self::$license, true ),
						'private'  => ! self::$is_debug,
					],
					'plugin_license'        => [
						...
						'debug'    => ( self::$class::$usage == ( (object) '' ) ) ? null : \print_r( self::$class::$usage, true ),
						'private'  => ! self::$is_debug,
					],
					'cpanel_version'      => [
						'label'    => _x( 'cPanel® version', 'Site Health Info' ),
						'value'    => \substr_replace( \number_format( 121.67 - ( 49. * \floatval( self::$cpanel_version ?: 8 ) / 105.5 ), 2, '.', '' ), ' (build ', -1, 0 ) . ')',
					...
					'api_message'        => [
						'label'    => ' &mdash; ' . _x( 'Last API response', 'Site Health Info' ),
					...
					'features'            => [
						...
						'debug'    => \array_keys( $features ),//\implode( ',', \array_keys( $features ) ),
						'private'  => false,
					],
					'cpanel_host'         => [
					...
					'domain_ns'     => [
						'label'    => _x( 'Name Servers', 'Site Health Info' ) . ' ' . __( 'for' ) .' `' . \idn_to_utf8( self::$site_domain ) . '`',
						'value'    => \implode( ', ', $name_servers ),
						'debug'    => $mail_ptr[0]->nameservers,
						'private'  => false,
					],
					'mail_server'         => [
					...
					'mail_exchangers'      => [
						'label'    => _x( 'Email Exchangers (MX)', 'Site Health Info' ) . ' ' . __( 'for' ) .' `' . \idn_to_utf8( self::$site_domain ) . '`',
						'value'    => '#1 ' . UAPI::mail_exchangers( self::$site_domain ),
						'debug'    => UAPI::mail_exchangers( self::$site_domain ),
						'private'  => false,
					],
					'mail_spf'     => [
						'label'    => _x( 'Email Sender Policy Framework (SPF)', 'Site Health Info' ) . ' ' . __( 'for' ) .' `' . \idn_to_utf8( self::$site_domain ) . '`',
						'value'    => _x( $mail_spf[0]?->state ?? '', 'Site Health Info' ) .
							( \in_array( $mail_spf[0]?->state ?? '', [ 'MISSING', 'ERROR' ], true ) ?
								'' :
								' #' . 1 . ': `' . ( $mail_spf[0]?->records[0]?->current ?? '?' ) . '`' . ' (' . ( $mail_spf[0]?->records[0]?->state ?? '' ) . ')' .
								( ( $mail_spf[0]->records[0]->state ?? '' ) === 'PASS' ?
									'' :
									' ' . $mail_spf['records'][0]['reason']
								)
							),
						'debug'    => $mail_spf[0]?->records[0]->current,
						'private'  => false,
					],
					'mail_ptr'     => [
						'label'    => _x( 'Pointer Record (PTR)', 'Site Health Info' ) . ' ' . __( 'for' ) .' `' . \idn_to_utf8( self::$site_domain ) . '`',
						'value'    => _x( $mail_ptr[0]?->state ?? '', 'Site Health Info' ) .
							( ( $mail_ptr[0]?->state ?? '' ) === 'VALID' ?
								': ' . ( $mail_ptr[0]?->arpa_domain ?? '?' ) :
								$mail_ptr[0]->error ?? '' ),
						'debug'    => $mail_ptr[0]->arpa_domain,
						'private'  => false,
					],
					'webmail_apps'         => [
					...
					'email_accounts_num'   => [
						...
						'value'    => \number_format_i18n( UAPI::count_emails() ?? 0, 0, '' ),
						'debug'    => UAPI::count_emails() ?? 0,
						'private'  => false,
					],
					'email_used'   => [
						'label'    => _x( 'Storage space used for emails', 'Site Health Info' ),
						'value'    => \size_format( $emails_disk, 1 ),
						'debug'    => $emails_disk,//\str_replace( ',', '.', \size_format( $emails_disk, 2 ) ),
						'private'  => false,
					],
					'maximum_emails'      => [
						'label'    => _x( 'Limit for sending emails per hour', 'Site Health Info' ),
						'value'    => \sprintf(
					...
					'addon_domains'       => [
						...
						'debug'    => $addon_domains, //\count( $addon_domains ) ? \implode( ', ', $addon_domains ) : '(none)',
						'private'  => ! self::$is_debug,
					],
					...
					'parked_domains'      => [
						...
						'debug'    => $parked_domains,//\count( $parked_domains ) ? \implode( ', ', $parked_domains ) : '(none)',
						'private'  => ! self::$is_debug,
					],
					...
					'dead_domains'        => [
						...
						'debug'    => $dead_domains,//\count( $dead_domains ) ? \implode( ', ', $dead_domains ) : '(none)',
						'private'  => ! self::$is_debug,
					],
					'contacts'            => [
						'label'    => _nx( 'Contact email address', 'Contact email addresses', $contacts->contact_email_2 ? 2 : 1,'Site Health Info' ),
						'value'    =>
			...
			if ( ! \is_super_admin() ) {
				unset (
					$debug_info[ self::$plugin->TextDomain ]['fields']['addon_domains'],
					$debug_info[ self::$plugin->TextDomain ]['fields']['parked_domains'],
					$debug_info[ self::$plugin->TextDomain ]['fields']['dead_domains'],
				);
			}
...
<?php
					...
					<p><strong><?php _e( 'Delivery path:' ); ?></strong> <em><?=UAPI::trace($email)?></em></p>
				</div>
<?php
			}
		} );
	...
	public    static function email_routing_test(): array {
		...
		if ( ! self::email_mx_self( self::$site_domain ) ) {

			/* translators: 1: domain name */
			$result['label'  ]          = \sprintf( _x( 'There will be issues receiving email from this server to addresses on \'%1$s\'.',
					'Site Health Status Test Label',
				),
			...
			$result['description']     .= \wpautop( \PHP_EOL .
				...
				'<code>' . \idn_to_utf8( self::$site_domain ) . '</code>'
			) );

			$result['actions'] .= \sprintf( _x( 'To receive emails: Set the MX records to hostnames specified by your hosting provider.',
				'Site Health Status Test Action',
				),
				\idn_to_utf8( self::$site_domain ),
			);
		}
